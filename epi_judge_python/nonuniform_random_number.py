...
def nonuniform_random_number_generation(values: List[int],
    ...
    accumulatedProbabilities = list(itertools.accumulate(probabilities))
    probabilityIdx = bisect.bisect_left(accumulatedProbabilities,random.random())
    return values[probabilityIdx]
